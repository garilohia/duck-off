...
const TILT_KEY='duckoff_tilt';
const touchDevice=()=>typeof window!=='undefined'&&('ontouchstart' in window||navigator.maxTouchPoints>0);
const readTilt=()=>{try{const v=localStorage.getItem(TILT_KEY);return v===null?touchDevice():v==='1'}catch{return touchDevice()}};
// iOS asks before sharing motion data; it must be requested from a tap.
async function requestTilt():Promise<boolean>{const D=(window as unknown as {DeviceOrientationEvent?:{requestPermission?:()=>Promise<string>}}).DeviceOrientationEvent;if(!D)return false;if(typeof D.requestPermission!=='function')return true;try{return (await D.requestPermission())==='granted'}catch{return false}}
...
export default function RaceScreen({race,players,legends,results,items,effects,features,identity,onTap,onStart,onUseItem,onSwitchLane,onLeave}:Props){
 ...
 const [tilt,setTilt]=useState(readTilt),tiltGranted=useRef(false),tiltArmed=useRef(true);
 ...
 const armTilt=async()=>{if(!tilt||tiltGranted.current)return;tiltGranted.current=await requestTilt();if(!tiltGranted.current)setTilt(false)};
 ...
 const tap=useCallback(()=>{
  ...
  lastTap.current=now;tapPending.current++;unlockAudio();void armTilt();
 ...
 // Tilt the phone to hop a lane (portrait uses gamma, landscape uses beta). A hop re-arms once the phone levels out.
 useEffect(()=>{
  if(!tilt||!canDrive)return;
  // The viewport shape decides the axis: portrait rolls about gamma, landscape about beta (sign follows which way the phone was turned).
  // One tilt is one hop: it re-arms only once the phone comes back near level, across re-renders.
  const onTilt=(e:DeviceOrientationEvent)=>{if(e.gamma==null&&e.beta==null)return;const landscape=window.innerWidth>window.innerHeight;const type=screen.orientation?.type??'landscape-primary';const angle=landscape?(e.beta??0)*(type==='landscape-secondary'?-1:1):(e.gamma??0);if(tiltArmed.current&&Math.abs(angle)>13){tiltArmed.current=false;switchLane(Math.sign(angle))}else if(!tiltArmed.current&&Math.abs(angle)<6)tiltArmed.current=true;};
  window.addEventListener('deviceorientation',onTilt);return()=>window.removeEventListener('deviceorientation',onTilt);
 },[tilt,canDrive,switchLane]);
 ...
 const laneRow=<div className="lane-row" aria-label="Lane"><span className="lane-dots" aria-hidden="true">{Array.from({length:LANES},(_,i)=><i key={i} className={i===mine?.lane?'on':''}/>)}</span><span className="lane-help">{touchDevice()?<button type="button" className={`tilt-toggle ${tilt?'on':''}`} aria-pressed={tilt} onPointerDown={e=>{e.stopPropagation()}} onClick={()=>{const next=!tilt;setTilt(next);try{localStorage.setItem(TILT_KEY,next?'1':'0')}catch{}if(next){tiltGranted.current=false;void armTilt()}}}>{tilt?'tilt to steer · on':'tilt to steer · off'}</button>:'← → to change lanes'}{touchDevice()&&<small>or swipe</small>}</span></div>;
 return <main className={`race-screen phase-${race.status}`} data-phase={race.status}>
  ...
  {finished&&!race.forfeited&&<div className="results-scroll"><section className="results-card surface" aria-label="Race results"><div className="eyebrow">A VERY GOOD LITTLE RACE</div><h1>{orderedResults[0]?.name??race.winnerName} wins!</h1><div className="winner-portrait"><DuckPreview index={orderedResults[0]?.duckIndex??race.winnerDuckIndex} spinnable/><span className="winner-medal">1</span></div><p className="result-message">{myResult?myResult.drowned?`The whirlpool took you at ${Math.floor(myResult.pos/24)}%. ${ordinal(myResult.place)} place, and a very good story.`:`You splashed into ${ordinal(myResult.place)}. ${myResult.place===1?'Look at you go!':'Your duck is proud of you.'}`:'Your little duck is up next.'}</p>
   <ol className="results-list" aria-label="Final rankings">{orderedResults.map(p=><li key={p.id} data-place={p.place} className={p.identity.toHexString()===identity?'is-you':''}><b className="result-place">{p.place<=3?['①','②','③'][p.place-1]:p.place}</b><span className="result-name">{p.name}{p.identity.toHexString()===identity&&<small>you</small>}<em>{p.pos>=TRACK?'Crossed the line':p.drowned?`Went under at ${Math.floor(p.pos/24)}%`:`${Math.floor(p.pos/24)}% at the buzzer`}</em></span><span className="result-taps">{p.taps}<small>taps</small></span></li>)}</ol>
