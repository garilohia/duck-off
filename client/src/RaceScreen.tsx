...
const needsMotionPrompt=()=>typeof (window as unknown as {DeviceOrientationEvent?:MotionCtor}).DeviceOrientationEvent?.requestPermission==='function';
async function requestTilt():Promise<boolean>{const D=(window as unknown as {DeviceOrientationEvent?:MotionCtor}).DeviceOrientationEvent;if(!D)return false;if(typeof D.requestPermission!=='function')return true;try{return (await D.requestPermission())==='granted'}catch{return false}}
...
export default function RaceScreen({race,players,legends,results,items,effects,features,identity,onTap,onStart,onUseItem,onSwitchLane,onLeave}:Props){
 ...
 const [motionGranted,setMotionGranted]=useState(()=>!needsMotionPrompt()),tiltArmed=useRef(true);
 ...
 // Tilt the phone to hop a lane. Portrait only: roll about gamma, one hop per tilt, re-armed once the phone levels out.
 useEffect(()=>{
  if(!canDrive)return;
  ...
  const onTilt=(e:DeviceOrientationEvent)=>{if(e.gamma==null||window.innerWidth>window.innerHeight)return;const angle=e.gamma;if(tiltArmed.current&&Math.abs(angle)>13){tiltArmed.current=false;switchLane(Math.sign(angle))}else if(!tiltArmed.current&&Math.abs(angle)<6)tiltArmed.current=true;};
 ...
 const laneRow=<div className="lane-row" aria-label="Lane"><span className="lane-dots" aria-hidden="true">{Array.from({length:LANES},(_,i)=><i key={i} className={i===mine?.lane?'on':''}/>)}</span><span className="lane-help">{touchDevice()?(motionGranted?'tilt to steer':<button type="button" className="tilt-toggle" onClick={()=>void armTilt()}>allow tilt steering</button>):'← → to change lanes'}{touchDevice()&&<small>or swipe</small>}</span></div>;
