...
export default function RaceScreen({race,players,legends,results,items,effects,features,identity,onTap,onStart,onUseItem,onSwitchLane,onSteer,onLeave}:Props){
 ...
 const [showHowTo,setShowHowTo]=useState(()=>!seenHowTo()),soloStarted=useRef(false);
 const solo=race.id.startsWith('SOLO-');
 ...
 const tap=useCallback(()=>{
  if(STEER_ONLY||!racing||spectator||mine?.place||drowned)return;const now=performance.now();if(now-lastTap.current<30||tapPending.current>=8)return;
 ...
 // A practice run starts by itself once the instructions are out of the way.
 useEffect(()=>{if(solo&&race.status==='lobby'&&!showHowTo&&!soloStarted.current){soloStarted.current=true;void onStart().catch(()=>{soloStarted.current=false})}},[solo,race.status,showHowTo,onStart]);
 ...
 const howTo=<div className="howto" role="dialog" aria-modal="true" aria-labelledby="howto-title"><section className="howto-card surface"><div className="eyebrow">BEFORE YOU PADDLE</div><h1 id="howto-title">How to play</h1><ul>
  ...
  <li><b>Swim into a toy to grab it.</b><span>Bomb, homing bubble, rocket or shield. Tap the toy button on the right{touchDevice()?', or shake the phone,':''} to use it. One at a time.</span></li>
  <li><b>Barge into a rival.</b><span>Slide into a duck beside you and they wobble. A shield blocks any hit.</span></li>
 </ul>{touchDevice()&&!motionGranted&&<p className="howto-note">Allow motion access when asked, that is what makes tilt steering work.</p>}<button className="primary" onClick={dismissHowTo}>{solo?'Start my run':'Got it'} <span aria-hidden="true">↗</span></button></section></div>;
 // The toy button floats on the centre-right so a thumb reaches it without leaving the paddle.
 const toyButton=racing&&!spectator&&!drowned&&!mine?.place?<button className={`toy-fab ${held?'has-item':''}`} disabled={!held||usingItem} onPointerDown={e=>e.stopPropagation()} onClick={()=>void useItem()} aria-label={held?`Use ${held.name}`:'No toy yet'} title={held?.hint}><span aria-hidden="true">{held?.icon??'🧸'}</span><b>{usingItem?'…':held?.name??'no toy'}</b></button>:null;
 return <main className={`race-screen phase-${race.status}`} data-phase={race.status} onPointerDown={e=>{if(!e.isPrimary||!(e.target instanceof Element)||e.target.closest('button,input,a,details,.results-scroll,.live-standings,.howto,.share-fallback,.connection-warning'))return;if(STEER_ONLY)switchLane(e.clientX<window.innerWidth/2?-1:1);else tap()}}>
