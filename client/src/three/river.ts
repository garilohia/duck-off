...
export const bendX = (z: number) => 9 * Math.sin(z * 0.028) + 4 * Math.sin(z * 0.075 + 1.3);
const slope = (z: number) => 9 * 0.028 * Math.cos(z * 0.028) + 4 * 0.075 * Math.cos(z * 0.075 + 1.3);
...
export function place(x: number, z: number): Placed {
  const heading = Math.atan(slope(z)),
    c = Math.cos(heading),
    s = Math.sin(heading);
  return { x: bendX(z) + x * c, z: z - x * s, heading };
}
...
export function ribbonGeometry(x0: number, x1: number, z0: number, z1: number, across: number, along: number, y = 0) {
  ...
  for (let j = 0; j <= along; j++) {
    ...
    for (let i = 0; i <= across; i++) {
      const p = place(x0 + ((x1 - x0) * i) / across, z);
      positions.push(p.x, y, p.z);
      uvs.push(i / across, j / along);
    }
  }
