...
const ball = new THREE.SphereGeometry(1, 10, 8),
  lowBall = new THREE.SphereGeometry(1, 7, 5),
  trunk = new THREE.CylinderGeometry(0.18, 0.28, 1, 7),
  stem = new THREE.CylinderGeometry(0.05, 0.07, 1, 5);
function Instances({
  geometry,
  color,
  items,
  ...
  return (
    <instancedMesh ref={ref} args={[geometry, undefined, items.length]} frustumCulled={false}>
      {flat ? <meshBasicMaterial color={color} /> : <meshStandardMaterial color={color} roughness={0.9} />}
    </instancedMesh>
  );
}
const GRASS = '#8fd18a',
...
function useLayout() {
  return useMemo(() => {
    ...
    const trunks: Item[] = [],
      ...
      hills: Item[] = [],
      pebbles: Item[] = [],
      fans: { p: V; side: number; heading: number }[] = [];
    for (const side of [-1, 1]) {
      for (let z = 18; z > -250; z -= 7 + r() * 5) {
        const kind = r();
        if (kind < 0.42) {
          const x = side * (27 + r() * 11),
            h = 2.2 + r() * 2.4,
            w = 1.5 + r() * 1.4;
          trunks.push({ p: at(x, h / 2 - 0.2, z), s: [1, h, 1] });
          canopies.push({ p: at(x, h + 0.1, z), s: [w, w * 0.85, w] });
          canopiesDark.push({ p: at(x + side * 0.5, h - 0.5, z + 0.4), s: [w * 0.7, w * 0.6, w * 0.7] });
          if (r() < 0.5) canopies.push({ p: at(x - side * 0.6, h + 0.8, z - 0.3), s: [w * 0.6, w * 0.55, w * 0.6] });
        } else if (kind < 0.68) {
          const x = side * (19 + r() * 5),
            w = 0.7 + r() * 0.8;
          bushes.push({ p: at(x, 0.15, z), s: [w, w * 0.7, w] });
          if (r() < 0.6) bushes.push({ p: at(x + side * 0.8, 0.1, z + 0.6), s: [w * 0.7, w * 0.5, w * 0.7] });
        } else if (kind < 0.8) {
          const x = side * (17.5 + r() * 4),
            w = 0.35 + r() * 0.6;
          rocks.push({ p: at(x, 0.05, z), s: [w, w * 0.6, w * 0.8], r: r() * 3 });
        } else {
          const x = side * (25 + r() * 12);
          for (let i = 0; i < 5; i++) {
            const target = [flowersPink, flowersYellow, flowersWhite][Math.floor(r() * 3)];
            target.push({ p: at(x + (r() - 0.5) * 3, 0.32, z + (r() - 0.5) * 3), s: [0.16, 0.16, 0.16] });
          }
        }
      }
      for (let z = 14; z > -245; z -= 4 + r() * 6) {
        const x = side * (15.4 + r() * 0.9);
        const count = 1 + Math.floor(r() * 3);
        for (let i = 0; i < count; i++) {
          const h = 0.9 + r() * 0.9,
            xx = x + (r() - 0.5) * 0.8,
            zz = z + (r() - 0.5) * 1.2;
          reeds.push({ p: at(xx, h / 2 - 0.05, zz), s: [1, h, 1], r: r() });
          if (r() < 0.5) cattails.push({ p: at(xx, h - 0.05, zz), s: [0.09, 0.22, 0.09] });
        }
      }
      for (let z = 16; z > -250; z -= 2 + r() * 3) {
        const w = 0.12 + r() * 0.16;
        pebbles.push({ p: at(side * (16.6 + r() * 2.2), 0.18, z), s: [w, w * 0.5, w * 0.8], r: r() * 3 });
      }
      for (let z = 0; z > -240; z -= 28 + r() * 14) {
        const zz = z - r() * 6,
          p = place(side * (18.6 + r() * 1.2), zz);
        fans.push({ p: [p.x, 0.35, p.z], side, heading: p.heading });
      }
      for (let z = 20; z > -300; z -= 34 + r() * 20)
        clouds.push({ p: at(side * (12 + r() * 40), 11 + r() * 6, z), s: [3 + r() * 3, 1.1 + r() * 0.6, 2 + r() * 1.5] });
      for (let z = 0; z > -320; z -= 30 + r() * 20)
        hills.push({ p: at(side * (46 + r() * 14), -4, z), s: [20 + r() * 12, 7 + r() * 5, 18 + r() * 10] });
    }
...
function Fans({ spots }: { spots: { p: V; side: number; heading: number }[] }) {
  const ref = useRef<THREE.Group>(null),
    passed = useRef<boolean[]>(spots.map(() => false));
  useFrame(({ clock, camera }) => {
    if (!ref.current) return;
    const t = clock.elapsedTime,
      duckZ = camera.position.z - 14;
    ref.current.children.forEach((fan, i) => {
      const dz = spots[i].p[2] - duckZ;
      const near = dz > -6 && dz < 18;
      const behind = dz > 1;
      if (behind && !passed.current[i] && dz < 12) {
        passed.current[i] = true;
        cheer(spots[i].side * 0.7);
      }
      if (dz < -40) passed.current[i] = false;
      const lift = near ? 0.7 : 0.22,
        rate = near ? 7 : 3;
      fan.position.y = spots[i].p[1] + Math.abs(Math.sin(t * rate + i * 1.3)) * lift;
      fan.rotation.z = Math.sin(t * rate + i * 1.3) * (near ? 0.28 : 0.12);
    });
  });
  return (
    <group ref={ref}>
      {spots.map((f, i) => (
        <group key={i} position={f.p} rotation={[0, (f.side < 0 ? 0.9 : -0.9) + f.heading, 0]} scale={0.4}>
          <mesh position={[0, 0.6, 0.1]} scale={[0.9, 0.7, 1]} geometry={ball}>
            <meshStandardMaterial color={PAL.yellow} roughness={0.35} />
          </mesh>
          <mesh position={[0, 1.35, -0.45]} scale={[0.8, 0.75, 0.72]} geometry={ball}>
            <meshStandardMaterial color={PAL.yellow} roughness={0.35} />
          </mesh>
          <mesh position={[0, 1.25, -1.15]} scale={[0.32, 0.12, 0.26]} geometry={ball}>
            <meshStandardMaterial color={PAL.beak} />
          </mesh>
          {[-1, 1].map((s) => (
            <mesh key={s} position={[s * 0.3, 1.45, -1.05]} scale={[0.11, 0.14, 0.06]} geometry={lowBall}>
              <meshBasicMaterial color="#152333" />
            </mesh>
          ))}
          <mesh position={[0, 0.1, 0]} rotation={[-Math.PI / 2, 0, 0]}>
            <circleGeometry args={[0.9, 12]} />
            <meshBasicMaterial color="#00000018" transparent depthWrite={false} />
          </mesh>
        </group>
      ))}
    </group>
...
export function Scenery() {
  ...
  const banks = useMemo(
    () => ({
      sandL: ribbonGeometry(-25, -16, 70, -330, 2, 100, 0.2),
      sandR: ribbonGeometry(16, 25, 70, -330, 2, 100, 0.2),
      grassL: ribbonGeometry(-72, -25, 70, -330, 3, 100, 0.3),
      grassR: ribbonGeometry(25, 72, 70, -330, 3, 100, 0.3),
      edgeL: ribbonGeometry(-25.6, -24.6, 70, -330, 1, 100, 0.32),
      edgeR: ribbonGeometry(24.6, 25.6, 70, -330, 1, 100, 0.32),
    }),
    [],
  );
  return (
    <group>
      ...
      <mesh geometry={banks.sandL}>
        <meshStandardMaterial color={PAL.sand} roughness={1} />
      </mesh>
      <mesh geometry={banks.sandR}>
        <meshStandardMaterial color={PAL.sand} roughness={1} />
      </mesh>
      <mesh geometry={banks.grassL}>
        <meshStandardMaterial color={GRASS} roughness={1} />
      </mesh>
      <mesh geometry={banks.grassR}>
        <meshStandardMaterial color={GRASS} roughness={1} />
      </mesh>
      <mesh geometry={banks.edgeL}>
        <meshStandardMaterial color={GRASS_DARK} roughness={1} />
      </mesh>
      <mesh geometry={banks.edgeR}>
        <meshStandardMaterial color={GRASS_DARK} roughness={1} />
      </mesh>
...
export function Water() {
  ...
  const geometry = useMemo(() => ribbonGeometry(-16, 16, 70, -330, 12, 160, -0.05), []);
  const uniforms = useMemo(
    () =>
      THREE.UniformsUtils.merge([
        ...
        {
          uTime: { value: 0 },
          uBlue: { value: new THREE.Color('#3d8fcf') },
          uDeep: { value: new THREE.Color('#1f5c9c') },
          uShallow: { value: new THREE.Color('#7ec1c9') },
          uFoam: { value: new THREE.Color(PAL.foam) },
          uSky: { value: new THREE.Color('#dce8ff') },
        },
      ]),
    [],
  );
  ...
  return (
    <mesh geometry={geometry}>
      <shaderMaterial toneMapped={false} fog ref={shader} uniforms={uniforms} vertexShader={VERT} fragmentShader={FRAG} />
    </mesh>
  );
}
