...
import {ItemBuoys,DuckAura,FlyingItem} from './ItemScene';
import type {RacePlayer,Race,RaceItem,ItemEffect} from '../module_bindings/types';
...
function Camera({target,x,lobby}:{target?:RacePlayer;x:number;lobby:boolean}){
 ...
 useFrame(({camera,size},dt)=>{const portrait=size.width<size.height,z=lobby?0:-(target?.pos??0)/10;
  // Follow from behind so the river, rivals and item buoys are visible ahead.
  destination.set(x+(lobby?2:1.6),lobby?5.6:7,z+(lobby?10:14));
  ...
  camera.lookAt(x,lobby?-2.1:0,z-(lobby?1:8));
...
function Water(){
 ...
 const uniforms=useMemo(()=>({uTime:{value:0},uBlue:{value:new THREE.Color(PAL.water)},uFoam:{value:new THREE.Color(PAL.foam)}}),[]);
 useFrame(({clock})=>{if(shader.current)shader.current.uniforms.uTime.value=clock.elapsedTime});
 return <mesh rotation={[-Math.PI/2,0,0]} position={[0,-.05,-120]}><planeGeometry args={[32,360,16,120]}/><shaderMaterial toneMapped={false} ref={shader} uniforms={uniforms} vertexShader={`varying vec2 vUv; uniform float uTime; void main(){vUv=uv;vec3 p=position;p.z+=sin(p.x*.8+uTime)*cos(p.y*.5+uTime*.7)*.035;gl_Position=projectionMatrix*modelViewMatrix*vec4(p,1.);}`} fragmentShader={`varying vec2 vUv;uniform float uTime;uniform vec3 uBlue;uniform vec3 uFoam;void main(){vec2 p=vUv*vec2(16.,180.);float t=uTime*.28;vec2 q=p+vec2(sin(p.y*.7+t),cos(p.x*.9-t))*.32;float a=sin(q.x*2.2+q.y*.5+t),b=sin(q.y*1.8-q.x*.6-t);float caustic=pow(1.-abs(a*b),12.);float ripple=sin(q.y*3.+sin(q.x*2.+t)+t);vec3 color=mix(uBlue*.91,uBlue*1.06,.5+.5*ripple);color=mix(color,uFoam,caustic*.15);float edge=smoothstep(.46,.5,abs(vUv.x-.5));color=mix(color,uFoam,edge*.55);gl_FragColor=vec4(color,1.);\n#include <tonemapping_fragment>\n#include <colorspace_fragment>}`}/></mesh>;
}
function Track(){
 return <><color attach="background" args={['#dce8ff']}/><fog attach="fog" args={['#dce8ff',45,125]}/><hemisphereLight args={['#ffffff','#dcad75',1.65]}/><directionalLight position={[-8,14,15]} intensity={2.5} color="#fff9f0"/><directionalLight position={[12,8,-10]} intensity={.8} color="#e4edff"/><Water/>{[-1,1].map(side=><group key={side}><mesh position={[side*26,-.3,-120]}><boxGeometry args={[20,1,360]}/><meshStandardMaterial color={PAL.sand}/></mesh>{Array.from({length:22},(_,i)=><group key={i} position={[side*15.8,.12,20-i*13]}><mesh rotation={[Math.PI/2,0,0]}><torusGeometry args={[.37,.14,8,16]}/><meshStandardMaterial color={i%2?PAL.yellow:PAL.paper}/></mesh></group>)}</group>)}<group position={[0,0,-240]}>{[-1,1].map(s=><mesh key={s} position={[s*15,5.5,0]}><cylinderGeometry args={[.23,.23,11,12]}/><meshStandardMaterial color={PAL.paper}/></mesh>)}<mesh position={[0,10.7,0]}><boxGeometry args={[30,1.4,.25]}/><meshStandardMaterial color={PAL.yellow}/></mesh>{Array.from({length:32},(_,i)=><mesh key={i} rotation={[-Math.PI/2,0,0]} position={[-15+(i%16)*2,.04,Math.floor(i/16)*2]}><planeGeometry args={[2,2]}/><meshBasicMaterial color={(i+Math.floor(i/16))%2?PAL.foam:PAL.deep}/></mesh>)}</group></>;
}
export default function RaceScene({race,players,items,effects,identity}:{race:Race;players:readonly RacePlayer[];items:readonly RaceItem[];effects:readonly ItemEffect[];identity:string}){
 ...
 return <SceneBoundary><Canvas camera={{position:[0,7,14],fov:54}} dpr={[1,1.4]} gl={{antialias:true,alpha:false,powerPreference:'high-performance',toneMapping:THREE.NeutralToneMapping,toneMappingExposure:1}}><Track/><Camera target={target} x={xFor(targetX)} lobby={lobby}/>{sorted.map((p,i)=><Racer key={p.identity.toHexString()} p={p} x={xFor(i)} mine={p.identity.toHexString()===identity} lobby={lobby} item={items.find(item=>item.identity.isEqual(p.identity))}/>)}{!lobby&&race.status!=='finished'&&<ItemBuoys nextPickup={items.find(item=>item.identity.toHexString()===(target?.identity.toHexString()??identity))?.nextPickup??0}/>} {effects.map(effect=><FlyingItem key={effect.id.toString()} effect={effect} sourceX={xFor(Math.max(0,sorted.findIndex(p=>p.identity.isEqual(effect.source))))} targetX={xFor(Math.max(0,sorted.findIndex(p=>p.identity.isEqual(effect.target))))} targetPos={sorted.find(p=>p.identity.isEqual(effect.target))?.pos??effect.targetPos}/>)}</Canvas></SceneBoundary>;
}
