...
function heater(w:number,h:number){const sh=new THREE.Shape();sh.moveTo(-w,h);sh.lineTo(w,h);sh.bezierCurveTo(w,h*.2,w*.55,-h*.55,0,-h);sh.bezierCurveTo(-w*.55,-h*.55,-w,h*.2,-w,h);return sh;}
const shieldGeometry=new THREE.ExtrudeGeometry(heater(.62,.72),{depth:.14,bevelEnabled:true,bevelSize:.04,bevelThickness:.04,bevelSegments:2});
const shieldRimGeometry=new THREE.ExtrudeGeometry(heater(.72,.82),{depth:.08,bevelEnabled:false});
...
function Toy({item}:{item:string}){
 return <group scale={1.3}>
  ...
  {item==='shield'&&<group rotation={[0,0,.15]}><mesh geometry={shieldGeometry} position={[0,-.55,-.08]}><meshPhysicalMaterial color={PAL.deep} roughness={.3} clearcoat={.7}/></mesh><mesh geometry={shieldRimGeometry} position={[0,-.55,-.1]}><meshStandardMaterial color={PAL.yellow}/></mesh><mesh position={[0,-.05,.1]} rotation={[Math.PI/2,0,0]}><cylinderGeometry args={[.22,.22,.08,5]}/><meshStandardMaterial color={PAL.yellow} emissive={PAL.yellow} emissiveIntensity={.25}/></mesh></group>}
 </group>;
}
