...
function Game(){
 ...
 useGameTools({connected:isActive,race:races[0],players:players.map(p=>({name:p.name,active:p.active,pos:p.pos,place:p.place,rank:p.rank,taps:p.taps}))},joinGame,()=>tap(),()=>start());
 ...
 return <RaceScreen race={races[0]} players={players} legends={legends} results={results.filter(r=>r.raceNumber===races[0].raceNumber)} identity={identity?.toHexString()??''} onTap={()=>tap()} onStart={()=>start()} onEdit={()=>setEditing(true)}/>;
}
