...
async function connect(name: string, i: number, selectedRoom = room, join = true) {
    ...
    DbConnection.builder()
      ...
      .onConnect((c) => {
        ...
        c.subscriptionBuilder()
          .onApplied(() => {
            void (async () => {
              if (join) await c.reducers.join({ name, duckIndex: i % 10, room: selectedRoom });
              clearTimeout(timer);
              resolve(c);
            })().catch(reject);
          })
...
async function main() {
  const first = await connect('Speedy', 0);
  await until(() => first.db.race.id.find(room)?.status === 'lobby');
  const stranger = await connect('Not joined', 0, room, false);
  await assert.rejects(stranger.reducers.startRace({}));
  ...
  await sleep(9000);
  assert.equal(first.db.race.id.find(room)?.status, 'lobby');
  console.log(`PASS: ${RACERS} players wait for a manual start; unjoined users cannot start.`);
  // A member who steps out and comes back cannot slip past the cap once the room filled behind them.
  {
    const onlineHere = () => [...first.db.player.iter()].filter((p) => p.room === room && p.online).length;
    const extras = await Promise.all([connect('Extra 1', 3, room), connect('Extra 2', 4, room)]);
    await until(() => onlineHere() === 12);
    const stepper = racers[RACERS - 1];
    await stepper.reducers.leaveRoom({});
    await until(() => onlineHere() === 11);
    const filler = await connect('Filler', 5, room);
    await until(() => onlineHere() === 12);
    await assert.rejects(
      stepper.reducers.join({ name: 'Duck back', duckIndex: 1, room }),
      /full/,
      'a returning duck is refused when the room filled up',
    );
    await filler.reducers.leaveRoom({});
    await until(() => onlineHere() === 11);
    await stepper.reducers.join({ name: `Duck ${RACERS}`, duckIndex: (RACERS - 1) % 10, room });
    await until(() => onlineHere() === 12);
    for (const c of extras) await c.reducers.leaveRoom({});
    await until(() => onlineHere() === RACERS);
    await until(() => [...first.db.racePlayer.iter()].filter((p) => p.room === room && p.active).length === RACERS);
    console.log('PASS: returning to a full room is refused; there is room again once someone leaves.');
  }
  ...
  assert.equal(first.db.race.id.find(room)?.raceNumber, 1);
  ...
  const safeLane = [0, 1, 2, 3, 4].find(
    (l) => ![...first.db.raceFeature.iter()].some((f) => f.room === room && f.kind === 'whirlpool' && f.lane === l),
  )!;
  ...
  await Promise.all(
    racers.map(async (c) => {
      ...
      for (let i = 0; i < 8 && lane() !== safeLane; i++) {
        const before = lane();
        await c.reducers.switchLane({ direction: safeLane > before ? 1 : -1 });
        await until(() => lane() !== before, 5000);
      }
      assert.equal(lane(), safeLane);
    }),
  );
  await until(() => racers.every((c) => first.db.racePlayer.identity.find(c.identity!)!.lane === safeLane), 5000);
  const duringCountdown = await connect('Countdown late', 6);
  assert.equal(duringCountdown.db.racePlayer.identity.find(duringCountdown.identity!)!.active, false);
  ...
  const late = await connect('Late Duck', 7);
  await late.reducers.tap({ count: 1 });
  assert.equal(late.db.racePlayer.identity.find(late.identity!)!.pos, 0);
  ...
  const started = Date.now(),
    last = Array(RACERS).fill(0),
    requests: Promise<void>[] = [];
  const timer = setInterval(() => {
    const now = Date.now();
    racers.forEach((c, i) => {
      const interval = i === 0 ? 60 : 95 + i * 8;
      if (now - last[i] >= interval) {
        last[i] = now;
        requests.push(c.reducers.tap({ count: 1 }));
      }
    });
  }, 20);
  try {
    await until(() => {
      ...
      assert(
        rows.every((p) => !p.drowned),
        'nobody drowns in the whirlpool-free lane: ' +
          JSON.stringify(rows.filter((p) => p.drowned).map((p) => ({ lane: p.lane, pos: Math.round(p.pos), safeLane }))),
      );
  ...
  assert(rankChecks > 20);
  assert.equal(results.length, RACERS);
  assert.deepEqual(
    results.map((p) => p.place),
    Array.from({ length: RACERS }, (_, i) => i + 1),
  );
  if (!AUTO_CRUISE) {
    assert(boosts);
    assert.equal(results[0].identity.toHexString(), first.identity!.toHexString(), 'the fastest tapper wins');
  }
  assert.equal(results[0].pos, 2400);
  const unfinished = results.filter((p) => p.pos < 2400);
  for (let i = 1; i < unfinished.length; i++) assert(unfinished[i - 1].pos >= unfinished[i].pos);
  for (const client of [...racers, late]) {
    await until(() => [...client.db.raceResult.iter()].filter((p) => p.room === room).length === RACERS);
    assert.deepEqual(
      results.map((p) => [key(p), p.place, p.pos]),
      [...client.db.raceResult.iter()]
        .filter((p) => p.room === room)
        .sort((a, b) => a.place - b.place)
        .map((p) => [key(p), p.place, p.pos]),
    );
  }
  console.log(
    `PASS: ${rankChecks} live ranking checks; ${AUTO_CRUISE ? 'cruising ducks finish' : 'fastest duck wins'}; ${RACERS} unique places; clients agree.`,
  );
  await first.reducers.join({ name: 'New room duck', duckIndex: 3, room: room + '-B' });
  racers[1].disconnect();
  await sleep(11000);
  assert.equal(late.db.race.id.find(room)?.status, 'finished');
  assert.equal([...late.db.raceResult.iter()].filter((p) => p.room === room).length, RACERS);
  assert.equal(late.db.race.id.find(room)?.raceNumber, 1);
  console.log('PASS: results survive disconnects and room changes, with no automatic rematch.');
  await until(() => {
    const host = late.db.race.id.find(room)?.hostIdentity;
    return !!host && host !== first.identity!.toHexString() && host !== racers[1].identity!.toHexString();
  });
  ...
  await newHost.reducers.startRace({});
  await until(() => late.db.race.id.find(room)?.raceNumber === 2);
  assert.equal(late.db.racePlayer.identity.find(late.identity!)!.active, true);
  assert.equal(late.db.racePlayer.identity.find(late.identity!)!.pos, 0);
  ...
  assert.equal(late.db.player.identity.find(late.identity!)?.online, false);
  assert.equal(late.db.racePlayer.identity.find(late.identity!), null);
  ...
  const idleStart = Date.now(),
    winsBefore = [...late.db.player.iter()].reduce((n, p) => n + p.racesWon, 0);
  await until(() => late.db.race.id.find(room)?.status === 'finished');
  const idleSeconds = (Date.now() - idleStart) / 1000;
  if (AUTO_CRUISE) {
    assert.equal(late.db.race.id.find(room)?.forfeited, false);
    assert(
      [...late.db.raceResult.iter()].some((r) => r.room === room),
      'cruising ducks still produce a podium',
    );
  } else {
    assert(idleSeconds > 13 && idleSeconds < 25, `forfeit after ~15s of stillness, took ${idleSeconds.toFixed(1)}s`);
    assert.equal(late.db.race.id.find(room)?.forfeited, true);
    assert.equal(late.db.race.id.find(room)?.forfeitReason, 'idle');
    assert.equal(late.db.race.id.find(room)?.winnerName, '');
    assert.equal([...late.db.raceResult.iter()].filter((r) => r.room === room).length, 0, 'a forfeited race has no podium');
    assert.equal(
      [...late.db.player.iter()].reduce((n, p) => n + p.racesWon, 0),
      winsBefore,
      'a forfeited race hands out no win',
    );
  }
  console.log(
    `PASS: leaving drops the duck from the roster, even mid-race; ${AUTO_CRUISE ? 'race 2 finishes on cruise' : 'a still race is forfeited with no winner'}.`,
  );
